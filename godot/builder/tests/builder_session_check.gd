...
func _check_recovery(session, golden: Dictionary) -> void:
	...
	_expect(bool(session.undo().get("ok", false)), "undo back to saved state succeeds")
	_expect(not session.has_unsaved_changes(), "undo back to saved state is clean")
	_expect(not session.has_recovery(), "clean undo removes stale recovery")
	var cancelled_flush: Dictionary = session.flush_recovery_for_test()
	_expect(not bool(cancelled_flush.get("written", false)), "clean undo cancels recovery debounce")
	_expect(bool(session.redo().get("ok", false)), "redo restores dirty state")
	var redone_flush: Dictionary = session.flush_recovery_for_test()
	_expect(bool(redone_flush.get("ok", false)) and bool(redone_flush.get("written", false)), "redo reschedules recovery")
	_expect(session.has_recovery(), "dirty redo is recoverable")
	var next_launch := SESSION_SCRIPT.new()
	root.add_child(next_launch)
	next_launch.initialize(session.author, NAMESPACE)
	_expect(next_launch.has_recovery(), "next launch discovers dirty recovery")
	next_launch.discard_recovery()
	next_launch.queue_free()
	var saved: Dictionary = session.save_document_as(SOURCE_PATH)
	...
	var prior_document: Dictionary = session.source_document.duplicate(true)
	var prior_history: bool = session.can_undo()
	var rejected: Dictionary = session.restore_recovery(func(_candidate: Dictionary) -> bool: return false)
	_expect(not bool(rejected.get("ok", false)), "rejected recovery hydration fails")
	_expect(session.source_document == prior_document, "rejected recovery preserves active document")
	_expect(session.can_undo() == prior_history, "rejected recovery preserves history")
	_expect(session.has_recovery(), "rejected recovery preserves snapshot")
